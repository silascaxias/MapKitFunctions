...
class MapViewController: UIViewController {
    ...
    @IBOutlet weak var switchFollowMode: UISwitch!
    ...
    var followModeState: Bool = false
    ...
    override func viewDidLoad() {
        ...
        switchFollowMode.addTarget(self, action: #selector(switchChanged), for:UIControl.Event.valueChanged)
    ...
    @IBAction func enlargeViewDidTapped() {
       guard let currentRange = mapView.cameraZoomRange else { return }
        mapView.setCameraZoomRange(MKMapView.CameraZoomRange(minCenterCoordinateDistance: currentRange.minCenterCoordinateDistance + 100.0, maxCenterCoordinateDistance: currentRange.maxCenterCoordinateDistance + 100.0), animated: true)
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "toDetailsPoint",
            let pointDetailsViewController = segue.destination as? PointDetailsViewController,
                let pointAnnotation = sender as? MKAnnotation {
            pointDetailsViewController.pointAnnotation = pointAnnotation
        }
    }
